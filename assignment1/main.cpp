...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  ...
  std::string line;
  std::getline(ifs, line); //Extract the first line of labels
  while (std::getline(ifs, line)){
    std::vector<std::string> splitVector =  split(line, ',');
    courses.push_back(Course{splitVector[0], splitVector[1], splitVector[2]});
  }
}
...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  //Delete the offereed courses from all courses
